...
class ProductTransformer:
    ...
    def transform_product(self, product: Dict, metafields: Dict, collections: Optional[List[str]] = None) -> List[Dict]:
        ...
        for variant in product.get('variants', []):
            # FILTRO: Skip variants with "personalizzazione" in options
            options = []
            if variant.get('option1'): options.append(variant.get('option1', '').lower())
            if variant.get('option2'): options.append(variant.get('option2', '').lower())
            if variant.get('option3'): options.append(variant.get('option3', '').lower())
            variant_options = ' '.join(options)
            
            if 'personalizzazione' in variant_options:
